...
arrdep = (
    r"\n(?P<action_code>[A-Z])"
    r"(?P<arrival_airline_designator>[A-Z]{2,3}|[A-Z][0-9]|[0-9][A-Z]|[A-Z]+)"
    r"(?P<arrival_flight_number>\d+)*"
    r"(?P<arrival_operational_suffix>[A-Z]{1,2})*"
    r"\s"
    r"(?P<departure_airline_designator>[A-Z]{2,3}|[A-Z][0-9]|[0-9][A-Z]|[A-Z]+)"
    r"(?P<departure_flight_number>\d+)*"
    r"(?P<departure_operational_suffix>[A-Z]{1,2})*"
    r"\s"
    r"(?P<period_of_operation_from>\d{2}[A-Z]{3})"
    r"(?P<period_of_operation_to>\d{2}[A-Z]{3}){0,1}"
    r"\s"
    r"(?P<days_of_operation>\d{7}){0,1}"
    r"\s"
    r"(?P<seats>\d{3})*"
    r"(?P<aircraft_type>\w{3})"
    r"\s"
    r"(?P<origin_station>[A-Z0-9]{3}){0,1}"
    r"(?P<previous_station>[A-Z0-9]{3})"
    r"(?P<scheduled_time_of_arrival_utc>\d{4})*"
    r"\s"
    r"(?P<scheduled_time_of_departure_utc>\d{4})*"
    r"(?P<overnight_indicator>[0-6]){0,1}"
    r"(?P<next_station>[A-Z0-9]{3})"
    r"(?P<destination_station>[A-Z0-9]{3}){0,1}"
    r"\s"
    r"(?P<arrival_service_type>[A-Z])"
    r"(?P<departure_service_type>[A-Z])"
    r"(?P<frequency_rate>\d){0,1}"
)

arr = (
    r"\n(?P<action_code>[A-Z])"
    r"(?P<arrival_airline_designator>[A-Z]{2,3}|[A-Z][0-9]|[0-9][A-Z]|\w{2}){0,1}"
    r"(?P<arrival_flight_number>\d+[A-Z]*|\w+){0,1}"
    r"(?P<arrival_operational_suffix>\w+){0,1}"
    r"\s"
    r"(?P<period_of_operation_from>\d{2}[A-Z]{3})"
    r"(?P<period_of_operation_to>\d{2}[A-Z]{3}){0,1}"
    r"\s"
...
dep = (
    r"\n(?P<action_code>[A-Z])"
    r"\s"
    r"(?P<departure_airline_designator>[A-Z]{2,3}|[A-Z][0-9]|[0-9][A-Z]|\w{2]){0,1}"
    r"(?P<departure_flight_number>\d+[A-Z]*|\w+){0,1}"
    r"(?P<departure_operational_suffix>\w+){0,1}"
    r"\s"
    r"(?P<period_of_operation_from>\d{2}[A-Z]{3})"
    r"(?P<period_of_operation_to>\d{2}[A-Z]{3}){0,1}"
    r"\s"
    r"("
    r"(?P<days_of_operation>\d{7})"
    r"\s"
    r"){0,1}"
    r"(?P<seats>\d{3})*"
    r"(?P<aircraft_type>\w{3})"
